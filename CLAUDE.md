...
**The nav carries the clock; a strip under it carries the routine.** `components/NavClock.tsx` is the time and date, sitting where the wordmark used to — the wordmark was the least useful thing in the bar, and the mark beside the clock still carries the identity and still links home. The date shortens rather than disappears (a bare time is ambiguous by the next morning): short below `xl`, long from there.
