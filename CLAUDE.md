...
**A status strip sits under the nav on every page** (`components/statusbar/`): clock, date, weather, location. It is the one part of the shell that can't be server-rendered — the clock would hydrate to a different minute and the location lives in `localStorage` — so it reserves `h-9` from first paint and fills in after mount, rather than growing and pushing the page down. Weather comes from **Open-Meteo** (no API key, so no new required secret) and place names from BigDataCloud's client reverse geocoder, both called from the browser; the app's own server never sees a coordinate. A **typed** location outranks geolocation, so the permission prompt happens once and can't silently relocate you afterwards.
