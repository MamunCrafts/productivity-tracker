...
**Every route opens with the same frame, and it lives in `components/PageFrame.tsx`.** `PageShell` is the measure and the gutters; `PageHeader` is the title block — `title`, `lead`, optional `eyebrow` (a back link) and optional `action` (the page's primary control). All six routes had this hand-written, and the phone half was wrong in the same way six times, so any rule about how a page opens is one edit here. The header is a **two-column grid that reflows rather than restacks**: below `sm` the action sits beside the title with the lead full width beneath both; from `sm` the title and lead stack into column one and the action spans both rows bottom-aligned. The flex column it replaced had to stretch the action to the full width of the screen to fill its row, and a 100%-wide amber bar two lines into the page shouts louder than anything it introduces. `/notes/[id]` keeps its own header — it carries a breadcrumb and up to four controls — but uses the same gutters.

**On a phone there are two ways to the same six routes, on purpose.** `NavMenu` is the drawer — full labels, plus the theme and sign-out controls that aren't destinations — and `components/MobileTabBar.tsx` is a fixed bottom tab bar that makes any page one tap from any other. All three surfaces read `LINKS` from `components/navLinks.ts`, so a new route appears in every one of them; `short` there is the abbreviated name for a tab's sixth of a 360px viewport (`tabLabel` falls back to `label`), while `label` stays the accessible name so the announcement never depends on available width.

**`--tabbar-h` in `app/globals.css` is how three things agree on that bar's height** — the bar reserves it, the layout's spacer at the foot of the page keeps the colophon clear of it, and `FocusTimer`'s docked bar lifts by it so a running session stacks *above* the destinations instead of burying them. It resolves to `0px` from `sm` up, so the desk layout is untouched and nothing needs a breakpoint of its own. Its media query is `max-width: 639.98px`, the exact complement of Tailwind's `sm`; a plain `639px` leaves a 1px band where the bar renders and nothing makes room for it. The spacer sits inside `ChromeOnly` rather than being padding on `<body>`, because the sign-in screens have no bar to make room for.

**Times are stored 24-hour and written 12-hour.** `startTime` is `HH:MM` in the database — that is what makes a day sortable as a plain string and the only value format `<input type="time">` accepts — and `lib/time.ts` (`to12Hour`, `formatTimeOfDay`) is the single place it becomes `6:30 am` for a reader. Every surface that prints a time goes through it, so the nav clock, the routine strip and `/routine` cannot drift. Never format a clock inline.
...
**The nav carries the wordmark and the clock.** `components/NavClock.tsx` is the time and date, beside a compacted wordmark — "Tracker" below `sm`, the full name from there, with the accessible name never shortening. The time is always shown; the date is what gives way for width. Below `sm` (tabs behind the menu button) it is the short form, from `xl` the long one, and in between there is no room for it beside six spelled-out tabs. Never format either inline — both go through `lib/time.ts`.

**`components/RoutineStrip.tsx` is the routine, on every page** — the block you are inside now, or when the next one starts if you are between them, linked to `/routine`. The question "what am I meant to be doing" gets asked constantly and in passing, which is the wrong shape for a page you have to navigate to. `routineNow(blocks, date)` in `lib/routine.ts` is the whole derivation and it has three cases that are each a bug if missed: a block starting at 23:00 is still current at 00:30 (yesterday's blocks are checked against a day-offset minute count), `next` can be tomorrow or later (the search walks forward a week, so a block that recurs on one weekday is still found), and overlaps are real, so `current` is the block ending soonest and `overlapping` counts the rest. The shell is now the first thing to need the routine, so it is what fetches it — hence `loaded` on the slice, since `status` returns to `"idle"` on success and can't tell "never asked" from "asked, and it's empty". The strip holds `h-9` open while that is unknown and collapses only once it knows there is nothing to say, so the common case has no layout shift.

**There is no weather and no location.** Both were removed on 2026-08-03 along with `lib/weather.ts`, `usePlaceWeather` and `LocationDialog`: they answered a question the app doesn't ask, and cost a geolocation prompt plus two third-party requests on every load. Don't reintroduce a browser-side call to Open-Meteo or a reverse geocoder — the app's server never saw a coordinate and nothing needs one now.

`/login` sits outside that set and outside the app shell: `components/ChromeOnly.tsx` returns null for it, so nav, docked timer and colophon don't render. It gates on the **route only**, never on signed-in state — the server can't read `localStorage`, so gating on that would strip the nav from every page's server-rendered HTML and let it pop in after hydration on every load.
...
- **The config is split in two, and that split is load-bearing.** `auth.config.ts` is proxy-safe and holds the `authorized` callback; `auth.ts` adds the Credentials provider, which needs mongoose and bcrypt. `proxy.ts` may only import the former so the request guard stays separate from database-backed sign-in.
- **Route protection lives entirely in proxy**, which is why no API route repeats the check. The matcher covers everything except `api/auth` (Auth.js's endpoints *and* `/api/auth/register`, which must work while signed out) and static assets. A signed-out request never reaches a handler or gets any markup.
...
**The board is three columns on a desk and one on a phone.** Below `sm` `ColumnSwitcher` picks which — a full-width segmented control carrying each column's count, so the whole board stays legible without swiping; from `sm` to `lg` the old snapping scroller of `w-72` columns; from `lg` a plain three-up grid. **Dragging is the desk gesture only.** On a phone the column a card is headed for is the one the switcher isn't showing, so there is nowhere on screen to drop it — each card instead carries two labelled steps (`sm:hidden`) that move it one column along, landing it at the foot of the column it arrives in, exactly where a drop on empty space would put it. `adjacentStatus` and `columnLabel` in `lib/board.ts` are the derivations behind them; the board is a line, so "move it along" is a step, not a choice from a list.
...
- **Tags come in two shapes and they are not the same object.** `TagFilter` on the shelf is a control: wrapped rather than scrolled, counted, cut at six with the rest one tap away, and the selected tag is always in the visible set — a filter you can't see is a filter you can't undo. `TagList` on a card and in the reader is a property of the note: quieter, no border, no count, nothing pressable, capped at four on a card and uncapped in the reader. Counts come from `tagCounts` in `lib/noteView.ts` over the **notes in the selected folder**, not the whole shelf, so the row can never offer a tag that returns nothing.
...
- **Nothing may exceed the viewport width.** Two past bugs came from fixed pixel sizes: the focus-mode breathing rings (now `min(420px,88vw)`) and the docked timer's control row. Prefer `min()`/`vw` over fixed `w-[Npx]`, and give wide content (tables, the heatmap) its own `overflow-x-auto`. The range filter used to be in that list; it is now a full-width four-column segmented control below `sm` with abbreviated labels from `RANGES[].short`, because a primary control that has to be scrolled sideways to reach "All time" is a worse answer than shorter words.
- **The page gutter is `px-4` on a phone and `px-6` from `sm`, and every full-bleed container has to match it.** A scroller that pulls itself out to the page edge does it with a negative margin — `-mx-4 px-4 sm:-mx-6 sm:px-6` — and mismatching the two is how content ends up 8px past the viewport on both sides with no visible cause. The board scroller, the week-summary table, the shimmer board and the note reader's tables all follow this; grep `-mx-` before changing a gutter.
- **No control that takes text may be under 16px on a phone.** iOS Safari zooms the page in on focus for any `input`, `textarea` or `select` below that, and it does not zoom back out — the page is simply left magnified. `components/ui/input.tsx` is `h-11 text-base` for this reason; hand-rolled fields (`NoteList`'s search, `CategoryTree`'s rename, the `CategoryPicker`/`HabitPicker` selects) carry `text-base sm:text-sm`.
- **The bottom tab bar is six equal columns, never a scroller.** Six fits 360px at 60px each, clearing the 44px touch floor in the dimension that gets tapped — and a shortcut you have to swipe sideways to reach isn't one. A seventh destination means rethinking the row, not shrinking the columns.
